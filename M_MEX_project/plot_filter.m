...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  min_H_mod = round(max(min(H_mod),min_amp));
  max_H_mod = round(min(max(H_mod),max_amp));
  min_H_arg = round(max(min(H_arg),min_phase));
  max_H_arg = round(min(max(H_arg),max_phase));
  ...
  % Bode of amplitude
  subplot (1,2,1);
  semilogx(f,H_mod,'LineWidth',width,'Color','r');
  % Information
  ...
  set   (gca,'ytick',(min_H_mod:amp_step:max_H_mod));
  grid   on;
  set   (gca,'XMinorGrid','off');
  % Bode of phase
  subplot (1,2,2);
  semilogx(f,H_arg,'LineWidth',width,'Color','r');
  % Information
  ...
  set   (gca,'FontName','Times','FontSize',police);
  set   (gca,'xtick',logspace(power_f_min,power_f_max,nb_decades));
  set   (gca,'ytick',(min_H_arg:phase_step:max_H_arg));
  grid   on;
  set   (gca,'XMinorGrid','off');
end
